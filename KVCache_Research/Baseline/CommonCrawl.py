...
from KVCache_Research.Baseline.utils import get_dataset
from KVCache_Research.Baseline.config import model_name, device, tokenizer, infer_baseline, max_length, num_experiments
import numpy as np

# 定义本地数据集路径
local_dataset_path = "./datasets/common_crawl"

# 加载测试集
dataset = get_dataset("cc_news", local_dataset_path, split="train")
# 过滤掉超过 max_length 的文本
texts = [text for text in dataset["text"] if len(tokenizer(text)["input_ids"]) <= max_length][:40]
...
for experiment in range(num_experiments):
    ...
    inference_times = []
    avg_memory_list, peak_memory_list = [], []  # 每次推理的显存占用

    for text in tqdm(texts, desc=f"实验 {experiment + 1}/{num_experiments}", unit="样本"):
        if len(text.strip()) > 0:
            metrics = infer_baseline(text)
...
print(f"Baseline - 平均 TTFT：{overall_metrics['TTFT']:.4f} 秒 (标准差: {std_metrics['TTFT']:.4f})")
print(f"Baseline - 平均 TPOT：{overall_metrics['TPOT']:.4f} 秒 (标准差: {std_metrics['TPOT']:.4f})")
print(f"Baseline - 平均 TBT：{overall_metrics['TBT']:.4f} 秒 (标准差: {std_metrics['TBT']:.4f})")
print(f"Baseline - 总体平均推理时间：{overall_avg_time:.4f} 秒 (标准差: {std_dev_time:.4f})")
print(f"Baseline - 平均显存占用：{overall_memory_usage['Avg_Memory_Usage']:.2f} MB")
print(f"Baseline - 峰值显存占用：{overall_memory_usage['Peak_Memory_Usage']:.2f} MB")